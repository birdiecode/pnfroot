...
Ptrace-движок поддерживает нативные 64-битные процессы Linux на `x86_64` и
`arm64` (`aarch64`). Платформа OCI-образа по умолчанию выбирается по архитектуре
хоста (`linux/amd64` или `linux/arm64`); при необходимости её можно переопределить
параметром `--image-platform`.
